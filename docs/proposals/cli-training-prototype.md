...
| `TreeNSGAIIOptimizingNSGAIIForBenchmarkRE3D` | Derivation-tree encoding |
...
The fourth case revealed that the flat and tree encodings are not just "different values for the same fields": the tree meta-optimizer operates directly on derivations of the base algorithm's own grammar, with two fixed operators (`SubtreeCrossover`, `TreeMutation`) instead of the flat encoding's operator catalogue (its meta-level parameter spaces, `NSGAIIMetaTree.yaml`/`AGEMOEAMetaTree.yaml`, only leave their probabilities, the mutation distribution index and the selection open). That is why `TrainingRequest` was split into two independent parts:
...
- **`MetaSearchConfig`** (sealed interface): how the meta-optimizer searches. Also not declared inline: the `metaSearch` field is the *name* of a reusable meta-optimizer configuration file (e.g. `metaSearch: MetaNSGAIIFlatConfiguration.yaml`), loaded by `MetaOptimizerConfigurationReader` from `src/main/resources/metaOptimizerConfigurations/`. Both variants declare an `algorithm` field (resolved by `MetaAlgorithmRegistry`, see below; `"NSGA-II"`, `"AGE-MOEA"`, `"SPEA2"`, `"AsyncNSGA-II"`, `"SMPSO"` and `"RandomSearch"` are registered for `flat`, `"NSGA-II"`, `"AGE-MOEA"` and `"RandomSearch"` for `tree`). None of the names carries a "Parallel" qualifier: all of them evaluate using `numberOfCores` (via `MultiThreadedEvaluation` or, for `AsyncNSGA-II`, its own asynchronous evaluation), so singling one out as "parallel" would be misleading, not informative:
  ...
  - `TreeMetaSearchConfig`: the same fields as `FlatMetaSearchConfig` (`metaPopulationSize` is not nullable: it defaults to 50 when absent), with `operatorFlags` parsed against a tree-specific parameter space (`crossoverProbability`, `mutationProbability`, `mutationDistributionIndex`, `selection`, ...).
...
classDiagram
    ...
    class MetaAlgorithmRegistry {
        ...
        +resolveTree(name, problem, config) EvolutionaryAlgorithm
        +resolveTreeRandomSearch(name, problem, config) RandomSearch
    }
    ...
    class TreeOutputResults {
        <<existing: org.uma.evolver.meta.output>>
    }
...
- The meta-optimizer algorithm is chosen explicitly via `metaSearch.algorithm`, resolved by `MetaAlgorithmRegistry`. For `flat` there are six registered engines, grouped into four `Family` values (none with a common jMetal supertype exposing `run()`/`result()`/`observable()` — they are duck-typed, not a shared interface):
  - `Family.EVOLUTIONARY` → `EvolutionaryAlgorithm`: `"NSGA-II"` (builds `DoubleNSGAII` directly, without going through `MetaNSGAIIBuilder`), `"AGE-MOEA"` (builds `DoubleAGEMOEA` directly, from `AGEMOEAMetaDouble.yaml`: the same operator catalogue as NSGA-II plus its `agemoeaVariant`) and `"SPEA2"` (via `MetaSPEA2Builder`, which hardcodes its own operators — SBX, polynomial mutation, strength ranking, KNN, tournament — and only exposes `populationSize`/`maxEvaluations`/`numberOfCores`/`mutationProbabilityFactor`, with no operator `ParameterSpace`).
  ...
  - `Family.RANDOM_SEARCH` → `RandomSearch`: `"RandomSearch"` (via `MetaRandomSearchBuilder`; no population and no operators, so a non-empty `operatorFlags` fails explicitly, same as SMPSO).

  For `tree`, three engines are registered (those whose descriptor declares `supportsTree`): `"NSGA-II"` and `"AGE-MOEA"`, built by `MetaAlgorithmRegistry.resolveTree` on `TreeNSGAII`/`TreeAGEMOEA` (Evolver algorithms on `DerivationTreeSolution`, configured through a `TreeParameterFactory` with `SubtreeCrossover` and `TreeMutation`), and `"RandomSearch"`, built by `resolveTreeRandomSearch`. `"SMPSO"` needs a `DoubleProblem`, and `"SPEA2"`/`"AsyncNSGA-II"` are built with `DoubleSolution` operators, so they stay flat-only. `MetaAlgorithmRegistry.familyOf(algorithm)` is the single source of truth for which algorithm shape each name returns, and `TrainingRunner` dispatches to `runFlat`/`runFlatAsync`/`runFlatPso`/`runFlatRandomSearch` (or, for `tree`, adapts the engine inside `runTree`) based on that classification. Note: `TrainingRunnerMain` ends with `System.exit(0)` because `AsynchronousMultiThreadedNSGAII` does not shut down its thread pool on its own — its internal `Worker`s are plain non-daemon `Thread`s in an infinite `while(true)` — (the same reason `example.training`'s `AsyncNSGAIIOptimizingNSGAIIForBenchmarkDTLZ` example already did the same, and why `TrainingRunnerSmokeIT`, see below, does not cover `AsyncNSGA-II`).
...
- The CLI does **not** resolve training sets by name (there is no `TrainingSetRegistry`): even though `org.uma.evolver.meta.trainingset.RE3DTrainingSet` already packages the 7 three-objective RE problems under the name "RE3D", `Re3dNSGAIIBaseLevel.yaml`/`Re3dNSGAIITreeBaseLevel.yaml` (and the generators that produce them) list them explicitly in `BaseLevelConfig`'s three parallel lists (the same approach `TrainingSet` already uses: problems, reference fronts, evaluations). This way no request is left with `null` fields waiting on "one shape or another", and there is no need to cross-reference `org.uma.evolver.meta.trainingset`'s subclasses to know what it actually runs. The trade-off is that `METADATA.txt` labels these cases `Problem Family: custom` instead of `RE3D`, since the CLI does not know that name.
- **Fixed for every meta-optimizer:** the offspring population size always equals the meta population size (each generation's offspring is evaluated in parallel as a whole), and the result is the final population, never an external archive (meta-level fronts usually hold very few solutions). Neither is configurable: `offspringPopulationSize` in a `metaSearch` file, or the legacy `metaOffspringSize` in a tree one, fails explicitly.
