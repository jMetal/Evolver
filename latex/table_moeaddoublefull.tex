...
\title{Parameter Space for MOEADDoubleFull Algorithm}
...
\begin{table}[!tb]
\caption{Parameter space of MOEADDoubleFull.}
\label{tab:params_moeaddoublefull}
\centering
\resizebox{\textwidth}{!}{%
\small
...
mutation & \Catdomain{uniform, polynomial, linkedPolynomial, nonUniform, levyFlight, powerLaw} \\
...
crossover & \Catdomain{SBX, blxAlpha, wholeArithmetic, arithmetic, laplace, fuzzyRecombination, PCX} \hspace{6em}\textbf{if} variation$=$crossoverAndMutationVariation \\
...
laplaceCrossoverScale & \Realdomain{0.1, 1.5} \hspace{6em}\textbf{if} crossover$=$laplace \\
...
This parameter space contains 37 parameters total, of which 29 are conditional parameters that are only active under specific conditions.
