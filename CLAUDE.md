...
| `org.uma.evolver.cli` | *Meta level.* Shared registries (base-level algorithms, problems, indicators) and status writer; `cli.training`: request/status/result runner for training jobs, used by Evolver-Studio; `cli.solving`: the same for running a configurable algorithm on a problem. |
