...
class NSGAIIDoubleTest {
  ...
  void setup() {
    ...
    nsgaIIDouble =
        new DoubleNSGAII(problem, populationSize, maximumNumberOfEvaluations, new YAMLParameterSpace("NSGAIIDouble.yaml", new DoubleParameterFactory()));
  }
  ...
  class ParseUnitTests {
    ...
    void shouldParseWorkProperlyWithTheDefaultSettings() {
      ...
      assertEquals(
          20.0,
          parameterSpace.get("polynomialMutationDistributionIndex").value(),
    ...
    void shouldParseWorkProperlyWithAConfigurationIncludingAnExternalArchive() {
      ...
      assertEquals(
          "bounds",
          parameterSpace.get("crossoverRepairStrategy").value(),
