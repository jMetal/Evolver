...
public class NSGAIIOptimizingNSGAIIForProblemsZCAT {
  ...
  public static void main(String[] args) throws IOException {
    ...
    List<DoubleProblem> trainingSet = List.of(new ZCAT2_2D(), new ZCAT6_2D(), new ZCAT12_2D(), new ZCAT14_2D(),
        new ZCAT19_2D());
    ...
    OutputResultsManagementParameters outputResultsManagementParameters = new OutputResultsManagementParameters(
        "NSGA-II", configurableProblem, "ZCAT", indicators,
        "RESULTS/NSGAII/ZCAT2D");

    var evaluationObserver = new EvaluationObserver(populationSize);
    var frontChartObserver =
        new FrontPlotObserver<DoubleSolution>(
            "NSGA-II, " + "ZCAT", indicators.get(0).name(),
            indicators.get(1).name(), "ZCAT", populationSize);
