 ...
 */
public class NSGAIIOptimizingRDEMOEAForProblemDTLZ3 {
  ...
  public static void main(String[] args) throws IOException {
    ...
    var outputResults =
        new OutputResults(
            ...
            indicators,
            "RESULTS/RDSMOEA/" + trainingSet.get(0).name());
    ...
    var frontChartObserver =
        new FrontPlotObserver<DoubleSolution>(
            "RDSMOEA, " + trainingSet.get(0).name(),
            indicators.get(0).name(),
            indicators.get(1).name(),
            trainingSet.get(0).name(),
            1);

    moea.observable().register(evaluationObserver);
    moea.observable().register(frontChartObserver);
    moea.observable().register(writeExecutionDataToFilesObserver);

    // Step 5: Run the meta-optimizer
    moea.run();

    // Step 6: Write results
    JMetalLogger.logger.info(() -> "Total computing time: " + moea.totalComputingTime());

    outputResults.updateEvaluations(maxEvaluations);
    outputResults.writeResultsToFiles(moea.result());
