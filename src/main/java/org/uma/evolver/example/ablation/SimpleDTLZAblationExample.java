 ...
 *
 * <p>This example shows how to run ablation analysis with minimal code using the {@link
 * AblationConfiguration} and {@link AblationRunner} classes.
 *
 * <p>The DTLZ configuration uses optimized parameters obtained from
 * AsyncNSGAIIOptimizingNSGAIIForBenchmarkDTLZ with the lowest IGD+ value after 2000 evaluations.
 *
 * <p>Usage examples:
 *
 * <ul>
 *   <li>{@code java SimpleDTLZAblationExample} - Run with default settings
 *   <li>{@code java SimpleDTLZAblationExample --threads 8} - Use 8 threads
 *   <li>{@code java SimpleDTLZAblationExample --sequential} - Force sequential execution
 *   <li>{@code java SimpleDTLZAblationExample --no-progress} - Disable progress reporting
 *   <li>{@code java SimpleDTLZAblationExample --runs 15} - Use 15 runs per configuration
 * </ul>
 *
...
public class SimpleDTLZAblationExample {
   ...
   */
  private static AblationConfiguration parseCommandLineArguments(
      AblationConfiguration config, String[] args) {
  ...
  private static void printUsage() {
    ...
    System.out.println(
        "  --runs <n>         Number of runs per configuration per problem (default: 10)");
    ...
    System.out.println();
    System.out.println(
        "Note: This example uses optimized DTLZ parameters from AsyncNSGAIIOptimizingNSGAIIForBenchmarkDTLZ");
  }
}
