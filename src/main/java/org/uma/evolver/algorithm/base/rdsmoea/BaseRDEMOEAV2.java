 ...
 */
public abstract class BaseRDEMOEAV2<S extends Solution<?>> implements BaseLevelAlgorithm<S> {
  ...
  /**
   * Constructs an AbstractNSGAII with the given population size and parameter space. This
   * constructor is typically used when the problem and maximum number of evaluations will be set
   * later.
   *
   * @param populationSize the population size to use
   * @param parameterSpace the parameter space for configuration
   */
  protected BaseRDEMOEAV2(int populationSize, ParameterSpace parameterSpace) {
    this.parameterSpace = parameterSpace;
    this.populationSize = populationSize;
  }

  /**
   * Constructs an AbstractNSGAII with the given problem, population size, maximum number of
   * evaluations, and parameter space. Initializes the ranking and density estimator components.
   *
   ...
   *
   * @return the solution creation strategy
   */
  ...
  protected Replacement<S> createReplacement() {
    ...
    //return new RankingAndDensityEstimatorReplacement<>(
        //preferenceForReplacement, replacementParameter.getRemovalPolicy());
  }
}
