 ...
 *
 * <ul>
 *   <li>Decomposition-based optimization using weight vectors
 *   <li>Configurable aggregation functions
 *   <li>Neighborhood-based mating and replacement
 ...
 * @see <a href="https://ieeexplore.ieee.org/document/4358754">Q. Zhang and H. Li, "MOEA/D: A
 *     Multiobjective Evolutionary Algorithm Based on Decomposition," in IEEE Transactions on
 *     Evolutionary Computation, vol. 11, no. 6, pp. 712-731, Dec. 2007, doi:
 *     10.1109/TEVC.2007.892759.</a>
...
public abstract class BaseMOEAD<S extends Solution<?>> implements BaseLevelAlgorithm<S> {
  ...
  /**
   * Constructs a new BaseMOEAD instance with the specified population size, weight vector
   * directory, and parameter space.
   *
   * <p>Note: This creates a partially configured instance. The {@link #createInstance(Problem,
   * int)} method must be called with a problem instance before the algorithm can be used.
   *
   * @param populationSize the size of the population. Must be positive.
   * @param weightVectorFilesDirectory the directory containing weight vector files. Must not be
   *     null.
  ...
  public BaseMOEAD(
    ...
    if (maximumNumberOfEvaluations <= 0) {
      throw new IllegalArgumentException(
          "Maximum number of evaluations must be positive: " + maximumNumberOfEvaluations);
    }
    if (maximumNumberOfEvaluations <= populationSize) {
      throw new IllegalArgumentException(
          "Maximum number of evaluations must be greater than population size");
    }
   ...
   *
   * <ul>
   *   <li>Termination condition based on maximum evaluations
   *   <li>Solution evaluation strategy
   *   <li>Initial population generation
   *   <li>Variation operators (crossover and mutation)
   *   <li>Selection mechanism for parent solutions
   *   <li>Replacement strategy for updating the population
   *   <li>Optional external archive for storing non-dominated solutions
   * </ul>
   *
  ...
  public EvolutionaryAlgorithm<S> build() {
    ...
    // Create builder with proper type parameter
   ...
   *
   * <p>This method configures the selection strategy based on the variation operator's requirements
   * for the number of parents and mating pool size.
   *
   ...
   *
   * <p>Subclasses must implement this method to provide the specific variation operator (typically
   * a combination of crossover and mutation) appropriate for the solution type.
   *
   ...
   *
   * <p>Subclasses must implement this method to provide the specific variation operator (typically
   * a combination of crossover and mutation) appropriate for the solution type. The implementation
   * should configure the variation operator using parameters from the parameter space and add any
   * required non-configurable parameters.
   *
   * @return a variation operator for creating new solutions
   */
   ...
   *
   * <p>This method retrieves the variation operator from the parameter space and adds the
   * subproblem ID generator as a non-configurable subparameter.
   *
   ...
   *
   * <p>This method creates a weight vector neighborhood based on the population size and number of
   * objectives in the problem. If the problem has two objectives, a simple weight vector
   ...
   * evaluations specified during construction. The algorithm will stop once this number of solution
   ...
   *
   * <p>This method retrieves the external archive configuration from the parameter space and sets
   * its size to match the population size.
   *
   ...
   *
   * <p>This method retrieves the solution creation strategy from the parameter space and configures
   * it with the problem and population size.
   *
   ...
   *
   * @param archive the external archive to update with evaluated solutions, or null if no archive
   *     is used
   ...
   *
   * <p>This method creates an evaluation component that can optionally update an external archive
   * with non-dominated solutions during the evaluation process. If no archive is provided, a simple
