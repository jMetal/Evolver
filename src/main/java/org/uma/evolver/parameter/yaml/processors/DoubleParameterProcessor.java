 ...
 *
 * <p>This processor handles double parameters defined with a range [min, max].
 * The range is required and must be specified as a list of two numbers.</p>
 */
public class DoubleParameterProcessor implements ParameterProcessor {
  /**
   * Creates a new DoubleParameterProcessor.
   */
  public DoubleParameterProcessor() {
  ...
  public void process(String parameterName, Object parameterConfig, ParameterSpace parameterSpace) {
    ...
    double[] range = extractAndValidateRange(parameterName, configMap);
    parameterSpace.put(new DoubleParameter(parameterName, range[0], range[1]));
  }
  ...
  private void validateNoValuesKey(String parameterName, Map<String, Object> configMap) {
    Check.that(!configMap.containsKey("values"), 
        "The 'values' key is not supported for double parameters. " +
  ...
  private void validateRangeExists(String parameterName, Map<String, Object> configMap) {
    Check.that(configMap.containsKey("range"),
        "No range defined for double parameter " + parameterName + ". " +
        "Use 'range: [min, max]'");
  }
   ...
   */
  private double[] extractAndValidateRange(String parameterName, Map<String, Object> configMap) {
