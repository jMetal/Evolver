...
/**
 * Training set for the DTLZ family configured for three-objective (3D)
 * problems.
 *
 * <p>
 * DTLZ problems can be defined for multiple objective dimensionalities;
 * therefore the class name
 * includes the dimensional suffix {@code 3D} to indicate the specific
...
public class DTLZ3DTrainingSet extends AbstractTrainingSet<DoubleSolution> {
  ...
  private static final List<Problem<DoubleSolution>> PROBLEMS = List.of(
      new DTLZ1(), new DTLZ2(), new DTLZ3(), new DTLZ4(), new DTLZ5(), new DTLZ6(), new DTLZ7());

  private static final List<String> REFERENCE_FRONT_FILE_NAMES = IntStream.range(1, PROBLEMS.size() + 1)
      .mapToObj(id -> "DTLZ" + id + ".3D.csv")
      .toList();
