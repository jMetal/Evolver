...
/**
 * This class represents a meta-optimization problem for configuring an algorithm based on quality
 * indicators. It extends the BaseMetaOptimizationProblem class.
 */
public class MetaOptimizationProblem extends BaseMetaOptimizationProblem {
  ...
  private String referenceFrontFileName;

  /**
   * Constructs a MetaOptimizationProblem with a configurable algorithm, a reference front file
   * name, and a list of quality indicators. Sets number of independent runs to 1.
   *
   * @param configurableAlgorithm  The builder for the configurable algorithm to be optimized.
   * @param referenceFrontFileName The name of the file containing the reference front for quality
   *                               indicators.
  ...
  /**
   * Constructs a MetaOptimizationProblem with a configurable algorithm, a reference front file
   * name, a list of quality indicators, and the number of independent runs.
   *
   * @param configurableAlgorithmBuilder The builder for the configurable algorithm to be
   *                                     optimized.
   * @param referenceFrontFileName       The name of the file containing the reference front for
   *                                     quality indicators.
   * @param indicators                   The list of quality indicators to be used in the
   *                                     optimization.
   * @param numberOfIndependentRuns      The number of independent runs to compute the median values
  ...
  public MetaOptimizationProblem(ConfigurableAlgorithmBuilder configurableAlgorithmBuilder,
    ...
    computeNormalizedReferenceFront(this.referenceFrontFileName);
  ...
  public MetaOptimizationProblem getNewInstance() {
      return new MetaOptimizationProblem(configurableAlgorithm, referenceFrontFileName, indicators, numberOfIndependentRuns) ;
  }
  ...
  /**
   * Evaluates the given solution by running the configurable algorithm with the decoded parameters
   * and computing the median values of quality indicators.
   *
   * @param solution The solution to be evaluated.
   * @return The solution with its objectives updated to the median values of the quality
   * indicators.
   */
  @Override
  public DoubleSolution evaluate(DoubleSolution solution) {
    ...
    IntStream.range(0, indicators.size())
        .forEach(i -> solution.objectives()[i] = medianIndicatorValues[i]);
  ...
  /**
   * Evaluates the given solution by running the configurable algorithm with the decoded parameters
   * and computing the median values of quality indicators over the number of independent runs.
   *
  ...
  private double[] computeIndependentRuns(String[] parameterArray) {
    ...
    for (int runId = 0; runId < numberOfIndependentRuns; runId++) {
      ...
      //String parameters = Arrays.stream(parameterArray).reduce("", (s1, s2) -> s1.concat(" " + s2));
