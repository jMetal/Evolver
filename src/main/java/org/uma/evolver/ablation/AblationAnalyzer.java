...
public class AblationAnalyzer<S extends Solution<?>> {
    ...
    private List<String> getConditionalSubParameters(String parameterName, String value) {
        try {
            ...
            if (parameter instanceof CategoricalParameter) {
                // CategoricalParameter doesn't directly expose a "get parameters for value"
                // method easily that returns names, but Parameter base class has
                // findConditionalParameters(value) which returns List<Parameter<?>>
    ...
    /**
     * Gets the set of all conditional sub-parameter names across all parameters.
     * This is used to
     * identify parameters that should be skipped in the main loop (because they are
     * handled as part
     * of their parent).
     */
    private Set<String> getAllConditionalSubParameters() {
        ...
        for (Parameter<?> param : parameterSpace.parameters().values()) {
            // For every parameter, check if it has conditional parameters
            // We need to iterate over all possible values of categorical parameters to find
            // all potential sub-parameters
            // Or easier: just look at all conditional parameters defined in the manager
            param
    ...
    private void applyConditionalParameters(
        ...
        // Remove sub-parameters from the old value
        ...
        // Add sub-parameters from the new target value
    ...
    /**
     * Performs leave-one-out ablation analysis across all problems. For each
     * parameter that differs
     * between configurations, reverts it to the default value and measures the
     * aggregated performance
     * loss across all problems.
     *
    ...
    /**
     * Performs forward ablation path analysis across all problems. Starting from
     * the default
     * configuration, greedily adds parameter changes one at a time, selecting the
     * change that gives
     * the best improvement when averaged across all problems.
     *
     * @param defaultConfig   the default/baseline configuration
    ...
    /**
     * Evaluates a configuration across all problems. Each problem's indicator
     * values are normalized
     * before aggregation to ensure comparable scales.
     *
