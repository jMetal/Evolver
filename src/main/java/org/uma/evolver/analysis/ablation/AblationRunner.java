...
/**
 * Generic runner for ablation analysis experiments.
 * Handles all the common logic for running ablation analyses with different configurations.
 * 
...
public class AblationRunner {
    ...
    public static class AblationResults {
        ...
        public AblationResults(AblationResult leaveOneOutResult, AblationResult forwardPathResult,
                             long leaveOneOutTime, long forwardPathTime) {
        ...
        public AblationResult getLeaveOneOutResult() { return leaveOneOutResult; }
        public AblationResult getForwardPathResult() { return forwardPathResult; }
        public long getLeaveOneOutTime() { return leaveOneOutTime; }
        public long getForwardPathTime() { return forwardPathTime; }
        public long getTotalTime() { return leaveOneOutTime + forwardPathTime; }
    }
    
    private void printHeader() {
        System.out.println(config.getAnalysisName() + " using " + config.getNumberOfThreads() + " thread(s)");
        if (config.getNumberOfThreads() == 1) {
            System.out.println("Sequential execution mode");
        } else {
            System.out.println("Parallel execution mode with " + config.getNumberOfThreads() + " threads");
        }
    ...
    private AblationAnalyzer<DoubleSolution> createAnalyzer(List<ProblemWithReferenceFront<DoubleSolution>> problems) {
        ...
        YAMLParameterSpace parameterSpace = new YAMLParameterSpace(
            config.getYamlParameterSpaceFile(), new DoubleParameterFactory());
        
        // Create algorithm
        DoubleNSGAII algorithm = new DoubleNSGAII(
            templateProblem, config.getPopulationSize(), config.getMaxEvaluations(), parameterSpace);
        
        // Create analyzer
        AblationAnalyzer<DoubleSolution> analyzer = new AblationAnalyzer<>(
            algorithm, problems, config.getIndicators(), config.getMaxEvaluations(), 
            config.getNumberOfRuns(), parameterSpace, config.getNumberOfThreads());
        
        // Set progress reporter if enabled
        if (config.isProgressReportingEnabled()) {
            analyzer.setProgressReporter(new ConsoleProgressReporter(
                config.isShowProgressBars(), config.isShowTimestamps()));
        }
        
        return analyzer;
    }
    
    private void printAnalysisInfo(List<ProblemWithReferenceFront<DoubleSolution>> problems,
        ...
        System.out.println("Problems: " + ProblemSuiteFactory.getProblemNames(config.getProblemSuite()));
        ...
        System.out.println("Total estimated algorithm executions: ~" + 
                          (problems.size() * config.getNumberOfRuns() * 
                           (defaultConfig.size() + optimizedConfig.size())));
        System.out.println();
    }
    
    private AblationResults runAnalyses(AblationAnalyzer<DoubleSolution> analyzer,
    ...
    private void validateConfiguration(Map<String, String> config, String configName) {
        ...
        String[] requiredParams = {"algorithmResult", "variation", "crossover", "mutation", "selection"};
        ...
        if (config.containsKey("populationSizeWithArchive")) {
            try {
                ...
                if (popSize < 10) {
                    System.err.println("Warning: " + configName + " has very small population size: " + popSize);
                }
    ...
    private void validateNumericParameter(Map<String, String> config, String paramName,
                                        double min, double max, String configName) {
        if (config.containsKey(paramName)) {
            try {
                double value = Double.parseDouble(config.get(paramName));
                if (value < min || value > max) {
                    System.err.println("Warning: " + configName + " parameter " + paramName +
                                     " value " + value + " outside expected range [" + min + ", " + max + "]");
                }
            } catch (NumberFormatException e) {
                System.err.println("Warning: Invalid numeric value for " + paramName + " in " + configName + " configuration");
            }
        }
    }
}