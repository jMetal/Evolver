...
/**
 * Factory for creating different problem suites for ablation analysis. Supports common benchmark
 * problem suites with their reference fronts.
 *
...
public class ProblemSuiteFactory {
    ...
    return switch (suiteName.toUpperCase()) {
      ...
      default ->
          throw new IllegalArgumentException(
              "Unsupported problem suite: " + suiteName + ". Supported suites: ZDT, DTLZ, WFG");
    };
  }

  /**
   * Creates the ZDT problem suite (ZDT1, ZDT2, ZDT3, ZDT4, ZDT6). Note: ZDT5 is excluded as it's a
   * binary problem.
   */
  ...
  /**
   * Creates the DTLZ problem suite (DTLZ1, DTLZ2, DTLZ3, DTLZ4, DTLZ7). Note: DTLZ5 and DTLZ6 are
   * excluded as they have degenerate Pareto fronts.
   */
  private static List<ProblemWithReferenceFront<DoubleSolution>> createDTLZProblemSuite()
    ...
    problems.add(
        new ProblemWithReferenceFront<>(
            new DTLZ1(),
            VectorUtils.readVectors(REFERENCE_FRONTS_PATH + "DTLZ1.3D.csv", ","),
            "DTLZ1"));

    problems.add(
        new ProblemWithReferenceFront<>(
            new DTLZ2(),
            VectorUtils.readVectors(REFERENCE_FRONTS_PATH + "DTLZ2.3D.csv", ","),
            "DTLZ2"));

    problems.add(
        new ProblemWithReferenceFront<>(
            new DTLZ3(),
            VectorUtils.readVectors(REFERENCE_FRONTS_PATH + "DTLZ3.3D.csv", ","),
            "DTLZ3"));

    problems.add(
        new ProblemWithReferenceFront<>(
            new DTLZ4(),
            VectorUtils.readVectors(REFERENCE_FRONTS_PATH + "DTLZ4.3D.csv", ","),
            "DTLZ4"));

    problems.add(
        new ProblemWithReferenceFront<>(
            new DTLZ7(),
            VectorUtils.readVectors(REFERENCE_FRONTS_PATH + "DTLZ7.3D.csv", ","),
            "DTLZ7"));
  ...
  /**
   * Creates the WFG problem suite (WFG1-WFG9 with 3 objectives). Note: This is a placeholder -
   * implement if WFG problems are needed.
   */
