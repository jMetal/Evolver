...
/**
 * Stores the results of an ablation analysis, including parameter contributions and the ablation
 * path.
 *
...
public class AblationResult {
  ...
  public Map<String, double[]> getPercentageContributions() {
    ...
    for (ParameterContribution pc : getRankedContributions()) {
      ...
      for (int i = 0; i < total.length; i++) {
        // Use a more robust threshold and handle edge cases
        if (Math.abs(total[i]) > 1e-8) {
          pct[i] = (pc.contribution()[i] / total[i]) * 100.0;
        } else {
          // When total improvement is near zero, contribution percentage is undefined
          pct[i] = Double.NaN;
        }
      }
  ...
  private String formatArray(double[] arr) {
    ...
    for (int i = 0; i < arr.length; i++) {
      if (Double.isNaN(arr[i])) {
        sb.append("N/A");
      } else if (Double.isInfinite(arr[i])) {
        sb.append(arr[i] > 0 ? "+∞" : "-∞");
      } else {
  ...
  public double[] getDefaultPerformance() {
    return defaultPerformance != null
        ? Arrays.copyOf(defaultPerformance, defaultPerformance.length)
        : null;
  }

  public void setDefaultPerformance(double[] p) {
    this.defaultPerformance = p != null ? Arrays.copyOf(p, p.length) : null;
  }

  public double[] getOptimizedPerformance() {
    return optimizedPerformance != null
        ? Arrays.copyOf(optimizedPerformance, optimizedPerformance.length)
        : null;
  }

  public void setOptimizedPerformance(double[] p) {
    this.optimizedPerformance = p != null ? Arrays.copyOf(p, p.length) : null;
  }

  public List<AblationStep> getAblationPath() {
    return ablationPath;
  }

  public void setAblationPath(List<AblationStep> path) {
    this.ablationPath = path;
  }

  public List<ParameterContribution> getContributions() {
